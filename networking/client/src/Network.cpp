...
namespace game_networking {
    ...
    Network::Network(sf::IpAddress& ip, unsigned short& port)  : connected(false)
    {
    ...
    void Network::disconnect(Player* player) // Disconnect the player from the server.
    {
        sf::Packet temp;
        temp << 1;
        temp << player->getID();
    ...
    void Network::send(Player* player)
    {
        sf::Packet temp;
        temp << 3;
        temp << player->getID();
        temp << player->getDirection().x;
        temp << player->getDirection().y;
    ...
    void Network::sendPosition(Player* player)
    {
        if(connected)
        {
            sf::Packet temp;
    ...
    void Network::sendMessage(Player* player, std::string& text)
    {
        sf::Packet temp;
        temp << 5;
        temp << player->getID();
    ...
    void Network::sendMyName(Player* player)
    {
        sf::Packet temp;
        temp << 6;
        temp << player->getID();
    ...
    void Network::getPlayerList(Player* player)
    {
        sf::Packet temp;
        temp << 7;
        temp << player->getID();
    ...
    void Network::receive(std::vector<std::unique_ptr<Enemy>>& enemies, Player* player)
    {
        ...
        if (connection.receive(receivePacket) == sf::Socket::Done)
        {
            ...
            if (type == 0) // You connected to server, get your ID.
            {
                ...
                {
                    player->setID(id);
                    std::cout << "I connected to server, my ID: " << player->getID() << std::endl;
                    this->sendMyName(player);
                    sf::sleep(sf::milliseconds(50));
                    this->getPlayerList(player);
                }
                connected = true;
            }
            else if (type == 1) // Disconnected!
            {
                for (unsigned int i = 0; i < enemies.size(); i++)
                {
                    ...
                    {
                        textMessage = "Player " + enemies[i]->getName() + " disconnected.";
            ...
            else if (type == 5) // Chat message received.
            {
                ...
                if (!receivedMessage.empty())
                {
                    ...
                    textMessage = senderName + ":" + newString;
                }
            }
            ...
            else if (type == 7) // Create new players
            {
                ...
                for (unsigned int i = 0; i < playersId.size(); ++i) //loop through id-s we got
                {
                    ...
                    if (playersId[i] != player->getID() && !haveThatEnemy) //if it is not our id and if we dont have that enemy, create a new enemy with that id
                    {
                        enemies.push_back(std::make_unique<Enemy>(playersId[i], sf::Vector2f(100, 100), playersName[i]));
                        textMessage = "New player connected: " + playersName[i];
